...
# 	D: Note, this is a really weird property. Setting it small doesn't change the size of 
# 	the selection display in demo_2d.tscn. All that happens is the granularity of the hex
#	grid changes; the selected hex indicator remains at the same size.
var hex_scale: Vector2
...
func _init():
	...
	# set_hex_scale(Vector2(1, 1))


static func obj_to_coords(val):
