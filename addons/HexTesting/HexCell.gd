...
var cube_coords = Vector3(0, 0, 0):
	set=set_cube_coords, get=get_cube_coords
# but other coord systems can be used
var axial_coords:
	set=set_axial_coords, get=get_axial_coords
var offset_coords:
	set=set_offset_coords, get=get_offset_coords


func _init(coords=null):
	# HexCells can be created with coordinates
	if coords:
		self.cube_coords = HexGrid.obj_to_coords(coords)
...
func set_cube_coords(val):
	...
	if abs(val.x + val.y + val.z) > 0.0001:
		...
		return
	cube_coords = HexGrid.round_coords(val)
...
func set_axial_coords(val):
	...
	set_cube_coords(HexGrid.axial_to_cube_coords(val))
...
func get_ring(distance):
	...
	var cells = Array()
	var current = new_hex(self.cube_coords + (HexGrid.DIR_N * distance))
	for dir in [HexGrid.DIR_SE, HexGrid.DIR_S, HexGrid.DIR_SW, HexGrid.DIR_NW, HexGrid.DIR_N, HexGrid.DIR_NE]:
...
func distance_to(target):
	...
	target = HexGrid.obj_to_coords(target)
	return int((
...
func line_to(target):
	...
	for dist in range(steps):
		...
		path.append(new_hex(HexGrid.round_coords(lerped)))
